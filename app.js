...
function typeByPercent(pct){if(pct<10)return"찍먹 클라이머";if(pct<20)return"취미 클라이머";
  if(pct<30)return"삐빅 정상입니다";
  if(pct<50)return"클밍 과몰입러";if(pct<70)return"클밍 중독자";if(pct<80)return"클밍 의존자";
  if(pct<90)return"자타공인 클친자";if(pct<97)return"ㄹㅈㄷ 클친자";return"일상생활 가능하세요?";}

const levelFromTotal=total=>total<=2?1:total<=6?2:total<=10?3:4;

const axisMetaByLevel={
  action:[{tag:"간헐적 방문자",hash:"#찍먹중"},{tag:"루틴 클라이머",hash:"#이번주암장은어디?"},{tag:"암장상주형",hash:"#암장으로출근중"},{tag:"암장 거주자",hash:"#암장에월세내기"}],
  mental:[{tag:"분리형",hash:"#현생유지중"},{tag:"잔상형",hash:"#이거그립좋은데?"},{tag:"점유형",hash:"#이건핀치…이건슬로퍼…"},{tag:"뇌내완등자",hash:"#헤헤돌이당"}],
  life:[{tag:"취미형",hash:"#취미클라이머"},{tag:"개입형",hash:"#일상에클밍한조각"},{tag:"잠식형",hash:"#클밍언제쉬지"},{tag:"생활재편성",hash:"#life_is_climb"}],
  growth:[{tag:"즐겜러",hash:"#클밍재밌당"},{tag:"유망주",hash:"#잘하고싶다"},{tag:"집착광공",hash:"#클밍중독"},{tag:"꽤나 진심인 편",hash:"#더이상취미의범주아님"}],
  pain:[{tag:"자기보호형",hash:"#아프면쉬기"},{tag:"관리형",hash:"#아프면살살"},{tag:"무시형",hash:"#의사가뭘알아!"},{tag:"연소형",hash:"#디져라_미래의나"}]
};

const axisDescByLevel={
  action:[["가끔 생각날 때만 감","클밍 갈 때 의지 필요"],["주기적으로 가야할 암장 있음","27연클 이런 거 들으면 그게 되나..? 당황스러움"],["어제도 가지 않았어? 언제 쉬어? 란 말 일상으로 듣는 중","날짜보다 어디 암장 간 날 이라고 이야기하는 게 더 빠름"],["너 선수할거야? 란 말 들어봄","주변 사람들이 주말/연휴에 뭐했는지 안 물어봄(클밍 했겠지...)"]],
  ...
  life:[["삶 != 클밍","클밍은 심심할 때"],["일상과 클라이밍 사이 밸런스가 좋음","클라이밍 운동 많이 된다."],["약속 잡을 때 클라이밍 일정 고려함","일정 중 시간 비면 근처 암장 괜히 검색해 봄"],["일상 중 다칠 뻔하면 '와 클밍 못할 뻔' 생각부터 듦","일정이 클밍으로 그득차 있음"]],
  growth:[["즐겜 마인드","완등하면 기분 좋음"],["잘하고 싶지만 행클이 더 중요!","주변에서 칭찬해 주면 기분 좋음"],["정체기 오면 침울해짐","행클도 중요하지만 빡클로 불 태워야 만족스러움"],["맛도리 문제 발견하면 눈 돌아감","실력에 만족? 절대 못 함"]],
  pain:[["아프면 쉼","몸 망가져가면서까지 하고 싶진 않음"],["건강한 클라이밍 중!","무리해서 하는 사람들보면 존경스러움"],["다치면 아픈 거 보다 클밍 쉬어야 하는 게 더 짜증남","도파민 터져서 아픈 거 잊음 다음날 몰려올 아픔은 알 바 아님"],["더 이상 건강 목적이 아님","병원 가면 쉬라고 할까봐 겁남"]]
};

const axisMainLineByLevel={
  action:["🗨️클밍 함 갈까~","🗨️이번 주엔 언제 가지","🗨️쉬는 거 그거 어케 하는건데","🗨️쉬엄쉬엄하면 쉰거지~"],
  mental:["🗨️클밍은 클밍, 일상은 일상","🗨️오 이 벽 잘하면 올라가겠는데?","🗨️클밍!하고!싶!다!","🗨️돌...만지고 싶다"],
  life:["🗨️심심한데 클밍이나 하러갈까~","🗨️클밍 넘 재밌졍","🗨️스킨만 회복되면 매일 하고 싶다","🗨️클라이밍이 일상"],
  growth:["🗨️재밌으면 된 겨~","🗨️될 듯 말 듯~ 재밌네","🗨️...한 번만 잡혀줘라","🗨️취미? 난 진심이야"],
  pain:["🗨️의사가 쉬랬어!","🗨️아무무가 되...","🗨️이 정도는 뭐","🗨️함 디져보자~"]
};
  ...
  if(tagsCol){
    ...
    tagsCol.querySelectorAll(".tag").forEach(t=>{
      ...
      if(s){
        ...
        s.style.transform = "translateY(-7px)";
      }
    });
  }
